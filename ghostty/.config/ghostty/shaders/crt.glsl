...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  col.r += 0.08 * texture(iChannel0, 0.75 * vec2(x + 0.025, -0.027) +
                                         vec2(uv.x + 0.001, uv.y + 0.001))
                      .x;
  col.g += 0.05 * texture(iChannel0, 0.75 * vec2(x + -0.022, -0.02) +
                                         vec2(uv.x + 0.000, uv.y - 0.002))
                      .y;
  col.b += 0.08 * texture(iChannel0, 0.75 * vec2(x + -0.02, -0.018) +
  ...
  float scans = clamp(
      0.35 + 0.35 * sin(3.5 * iTime + uv.y * iResolution.y * 1.5), 0.0, 1.0);
  ...
  col *=
      1.0 - 0.65 * vec3(clamp((mod(fragCoord.x, 2.0) - 1.0) * 2.0, 0.0, 1.0));
